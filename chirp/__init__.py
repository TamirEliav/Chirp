"""Chirp — Sound Analysis & Recording (package root).

During the Phase 1 monolith-to-package refactor, the entire contents of
the original `chirp.py` live in this `__init__.py`. Subsequent commits
extract subsystems (DSP, audio, recording, config, UI) into their own
submodules and re-export them from here for backward compatibility, so
existing imports (`from chirp import ChirpWindow`) continue to work.

The current status of the refactor is tracked in
`~/.claude/plans/robust-yawning-spring.md`.
"""
...
# Constants, color palette, and QSS theme live in chirp.constants. The
# star-import is intentional so every class below continues to see the
# old module-level names (SAMPLE_RATE, CHUNK_FRAMES, C, QSS, etc.)
# without editing every reference site in this transitional commit.
...
# ──────────────────────────────────────────────────────────────────────────────
# ThresholdRecorder was hoisted into chirp.recording.trigger in the Phase 1
# refactor (plan: c04). Re-exported here so existing `chirp.ThresholdRecorder`
# imports and the tests/test_trigger_characterization.py monkeypatch continue
# to work unchanged.
# ──────────────────────────────────────────────────────────────────────────────
from chirp.recording.trigger import ThresholdRecorder  # noqa: E402


# ─────────────────────────────────────────────────────────────────────────────
# RecordingEntity was hoisted into chirp.recording.entity in the Phase 1
# refactor (plan: c06). Re-exported so  keeps working.
# ─────────────────────────────────────────────────────────────────────────────
